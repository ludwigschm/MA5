...
class TabletopRoot(FloatLayout):
    ...
    def _complete_session_start(self, _dt: float = 0.0) -> None:
        ...
        payload = self._pending_session_start_payload
        if not payload:
            # Kein Payload: als "dirty" markieren und nächsten Check planen
            self._bridge_state_dirty = True
            # nutze _dt als "now", falls übergeben; sonst eigene Clock
            now = _dt if _dt else self._now()
            self._next_bridge_check = now + self._bridge_check_interval
            return
        else:
            # Achtung: dieses else ist optional, da oben returnt wird.
            pass
